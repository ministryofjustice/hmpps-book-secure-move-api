...
module FrameworkNomisMappings
  class Importer
    attr_reader :person, :assessment, :framework_responses, :framework_nomis_codes

    def initialize(assessment:)
      @assessment = assessment
      @person = assessment&.profile&.person
      @framework_responses = assessment&.framework_responses
      @framework_nomis_codes = framework_responses&.includes(:framework_nomis_codes)&.flat_map(&:framework_nomis_codes)
    end

    def call
      return unless assessment && framework_responses.any? && framework_nomis_codes.any?
      ...
      ApplicationRecord.retriable_transaction do
        ...
        assessment.update(nomis_sync_status: nomis_sync_status)
      end
    end
