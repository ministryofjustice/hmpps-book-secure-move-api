...
module Moves
  class Exporter
    ...
    def call
      Tempfile.new('export', Rails.root.join('tmp')).tap do |file|
        ...
        csv << headings
        moves.includes(:generic_events, :supplier).find_each do |move|
  ...
  private
    ...
    def attributes_row(move)
      ...
      cancellation_event = move.generic_events.order(created_at: :desc).where(type: 'GenericEvent::MoveCancel').first
      ...
      row = [
        ...
        move.journeys.select(&:billable?).any?,
    ...
    def cancellation_difference(move, cancellation_event)
      return unless move.cancelled?
      return if cancellation_event.blank?

      difference_in_seconds = (move.date.to_time.advance(hours: 9) - cancellation_event&.occurred_at)
      ...
      "#{cutoff} (#{(hours / 24).floor}d #{sprintf('%02d', (hours % 24))}h #{sprintf('%02d', (minutes % 60))}m)"
    end
  end
end
