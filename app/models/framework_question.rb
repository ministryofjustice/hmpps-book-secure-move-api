...
class FrameworkQuestion < VersionedModel
  ...
  def build_responses(question: self, assessment:, questions:, previous_responses: {})
    response = build_response(question, assessment, previous_responses[question.key])
    ...
    question.dependents.each do |dependent_question|
      ...
      dependent_response = build_responses(question: questions[dependent_question.id], assessment: assessment, questions: questions, previous_responses: previous_responses)
  ...
  def build_response(question, assessment, previous_response = nil)
    klass =
    ...
    klass.new(framework_question: question, assessmentable: assessment, value: previous_response, prefilled: previous_response.present?)
  end
end
