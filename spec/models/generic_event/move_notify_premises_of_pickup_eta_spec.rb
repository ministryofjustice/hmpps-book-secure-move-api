...
RSpec.describe GenericEvent::MoveNotifyPremisesOfPickupEta do
  subject(:generic_event) { build(:event_move_notify_premises_of_pickup_eta, eventable: move, occurred_at:) }
  ...
  it { expect(build(:event_move_notify_premises_of_pickup_eta, location_id: create(:location).id)).to validate_inclusion_of(:eventable_type).in_array(%w[Move]) }
