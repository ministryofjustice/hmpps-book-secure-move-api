...
RSpec.describe Api::FrameworkResponsesController do
  ...
  describe 'PATCH /youth_risk_assessments/:per_id/framework_responses' do
    include_context 'with supplier with spoofed access token'

    subject(:bulk_update_framework_responses) do
      patch "/api/youth_risk_assessments/#{per_id}/framework_responses", params: bulk_per_params, headers: headers, as: :json
    end

    let(:schema) { load_yaml_schema('patch_framework_response_responses.yaml') }
    let(:response_json) { JSON.parse(response.body) }
    let(:youth_risk_assessment) { create(:youth_risk_assessment, :in_progress) }
    let(:per_id) { youth_risk_assessment.id }
    ...
    context 'when successful' do
      ...
      context 'when responses are combined' do
        let(:string_response) { create(:string_response, assessmentable: youth_risk_assessment, value: 'No') }
        let(:array_response) { create(:array_response, assessmentable: youth_risk_assessment) }
        let(:object_response) { create(:object_response, :details, assessmentable: youth_risk_assessment) }
        let(:details_response) { create(:collection_response, :details, assessmentable: youth_risk_assessment) }
        let(:multiple_items_response) { create(:collection_response, :multiple_items, framework_question: multiple_question, value: nil, assessmentable: youth_risk_assessment) }
    ...
    context 'when unsuccessful' do
      ...
      context 'with a nested invalid value' do
        let(:framework_response) { create(:collection_response, :multiple_items, assessmentable: youth_risk_assessment) }
        let(:framework_question) { framework_response.framework_question.dependents.first }
        let(:other_framework_response) { create(:collection_response, :multiple_items, assessmentable: youth_risk_assessment) }
        let(:other_framework_question) { other_framework_response.framework_question.dependents.first }
      ...
      context 'when youth_risk_assessment confirmed' do
        ...
        let(:detail_403) { "Can't update framework_responses because youth_risk_assessment is confirmed" }
