  ...
  describe 'GET /moves' do
    ...
    describe 'filters' do
      ...
      it 'returns moves selected by from location' do
        get '/moves', headers: valid_headers, params: { filter: { from_location_id: move.from_location_id } }
        expect(JSON.parse(response.body)).to include_json(data: [{ id: move_id }])
      end

      it 'does not return moves filtered out by from location' do
        get '/moves', headers: valid_headers, params: { filter: { from_location_id: Random.uuid } }
        expect(JSON.parse(response.body)).not_to include_json(data: [{ id: move_id }])
      end

      it 'returns moves selected by to_location_type' do
        get '/moves', headers: valid_headers, params: { filter: { location_type: move.to_location.location_type } }
        expect(JSON.parse(response.body)).to include_json(data: [{ id: move_id }])
      end

      it 'does not return moves filtered out by to_location_type' do
        get '/moves', headers: valid_headers, params: { filter: { location_type: 'hospital' } }
        expect(JSON.parse(response.body)).not_to include_json(data: [{ id: move_id }])
      end

      it 'returns moves selected by date range' do
        filters = { date_from: Date.today.to_s, date_to: 5.days.from_now.to_date.to_s }
        get '/moves', headers: valid_headers, params: { filter: filters }
        expect(JSON.parse(response.body)).to include_json(data: [{ id: move_id }])
      end
      ...
      it 'returns moves selected by status' do
        get '/moves', headers: valid_headers, params: { filter: { status: move.status } }
        expect(JSON.parse(response.body)).to include_json(data: [{ id: move_id }])
      end

      it 'does not return moves filtered out by status' do
        get '/moves', headers: valid_headers, params: { filter: { status: 'not_a_status' } }
        expect(JSON.parse(response.body)).not_to include_json(data: [{ id: move_id }])
      end
    end
  end
